...
def aggregate_private_teacher_votes(teacher_responses, m, eps, delta_0, delta, aggregation_method='optimized'):
    eps_final = m * eps
...
def aggregate_public_teachers_GNMax(teacher_responses, sigma):
    # now aggregate teacher votes and compute a private final output
    K, n_response = teacher_responses.shape
    aggregated_votes = []
    for i in range(n_response):
        teacher_answers = teacher_responses[:, i]
...
def main():
    ...
    save_folder = 'private_mnist_teachers_test'
    delta = m * delta_0
    ...
    for exp_idx in range(num_exp):
        ...
        for agg_method in aggregation_methods:
            aggregated_votes = aggregate_private_teacher_votes(teacher_responses=teacher_responses, m=m, eps=eps,
