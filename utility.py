...
def compute_estimated_TV_distance(gamma_fn, T=10000, p_dist="uniform"):
    est_obj_val = 0
    K = len(gamma_fn)
    for trial in range(T):
        ...
        if p_dist == "uniform":
            ...
            # p_vec = [np.random.choice([np.random.uniform(0, 0.1), np.random.uniform(0.9, 1)]) for _ in range(K)]
            p_vec = [np.random.uniform(0, 0.1) for _ in range(K)]
        elif p_dist == "disagree":
            p_vec = [0.5 for _ in range(K)]
        else:
            raise Exception('Unknown p distribution {}!'.format(p_dist))
